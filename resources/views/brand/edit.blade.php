...
<body id="page-top">
    <div id="wrapper">
         ...
         @include('partials.admin_header2')
                <div class="container-fluid">
                    <div class="d-sm-flex justify-content-between align-items-center mb-4">
                        <h3 class="text-dark mb-0">Edit Brand</h3>
                    </div>
                    <div style="background: rgb(230,247,236);padding: 11px;">
                        <h1 style="font-size: 23px;">Basic Info</h1>
                       
                         {{-- START --}}
                        <form method="post" action="{{ route('brand.update', $brands[0]->brand_id) }}">
                            @csrf
                            {{ method_field('PATCH') }}
                            
                            <div class="form-row">
                                <div class="col"><label style="color: rgb(0,0,0);">Name:</label>
                                    <div class="form-group">
                                        <input class="form-control" type="text" name="brand_name" value="{{ $brands[0]->brand_name }}">

                                        @if($errors->has('brand_name'))
                                            <small><i style="color: red;">*{{ $errors->first('brand_name') }}</i></small>
                                        @endif
                                    </div>
                                </div>
                            </div>

                            <div><label style="color: rgb(0,0,0);">Category:</label>
                                <div class="form-group">
                                    <select class="form-control" name="category_id">
                                        <optgroup label="Category Names">
                                            <option value="">Select Category</option>

                                            @foreach($categories as $category)
                                                @if( $brands[0]->category_id == $category->category_id)
                                    ...
                                    @if($errors->has('category_id'))
                                        <small><i style="color: red;">*{{ $errors->first('category_id') }}</i></small>
                                    @endif
                                </div>
                            </div>
        ...
        </div><a class="border rounded d-inline scroll-to-top" href="{{ route('brand.index') }}"><i class="fas fa-arrow-left"></i></a>
    </div>
